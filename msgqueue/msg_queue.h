...
struct Message {
    ...
    Message(time_t t, int p, MessageType mt) : triggerTime(t), priority(p), type(mt) {}
};
...
class MessageQueue {
   ...
   public:
    ...
    void removeMessage(int id) { deletedIds.insert(id); }

    void setDoNotDisturb(bool enable) { doNotDisturb = enable; }
